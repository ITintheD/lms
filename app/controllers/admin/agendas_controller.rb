module Admin
	class AgendasController < AdminController
	  ...
	  def create
		...
		respond_to do |format|
		  if @agenda.save
		  send_mail(APP_CONFIG["test_email"], "#{@agenda.week.title} Agenda: #{@agenda.title}", "#{@agenda.body}") if params[:mail][:flag]
	  ...
	  def update
		...
		respond_to do |format|
		  ...
		  send_mail(APP_CONFIG["test_email"], "#{@agenda.week.title} Agenda: #{@agenda.title}", "#{raw(@agenda.body)}") if params[:mail][:flag]  
